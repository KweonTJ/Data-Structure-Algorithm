...
template <typename T>
auto dijksta_shortest_path(const Graph<T>& G, unsigned src, unsigned dst)
{
	...
	while (!heap.empty())
	{
		...
		{
			cout << current_vertex.ID << "번 저점에 안착!" << endl;
...
int main()
{
	...
	auto shortest_path = dijksta_shortest_path<T>(G, 1, 6);
